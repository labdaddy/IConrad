...
3. Create a VM to provide named and dhcpd service to your entire environment. Set up the dhcp daemon to use the ~~Spacewalk~~ Foreman server as the pxeboot machine (thus allowing you to use Cobbler to do unattended OS installs). Foreman [quickstart guide](https://theforeman.org/manuals/2.1/quickstart_guide.html). NOTE: From the Foreman/ Katello page, you must install Katello WITH Foreman. Katello cannot be installed after the fact. [Link.](https://theforeman.org/plugins/katello/3.16/installation/index.html)
